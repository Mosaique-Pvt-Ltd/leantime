                    ...
                    <form action="" method="post">
                        <?php if (count($this->get('allCanvas')) > 0) { ?>
                            <select data-placeholder="<?php echo $this->__("input.placeholders.filter_by_sprint") ?>"
                                    name="searchCanvas"
                                    class="mainSprintSelector mainSprintSelector-select" style="padding-left:0px; min-width: 200px;" onchange="form.submit()">
                            <?php
                            $lastClient = "";
                            $i = 0;
                            ...
                            <small><a href="javascript:void(0)"
                                      class="addCanvasLink" style="color: gray;"><?php echo $this->__("links.create_idea_board") ?></a></small> |
                         <small><a href="javascript:void(0)"
                                   class="editCanvasLink " style="color: gray;"><?php echo $this->__("links.edit_idea_board") ?></a></small>
                                   |
                                   <small>
                                        <?php  if ($login::userIsAtLeast("clientManager")) { ?>
                                        <a href="<?=BASE_URL ?>/ideas/delCanvas/<?php echo $this->get('currentCanvas') ?>"
                                        class="delete left" style="color: gray;"><?php echo $this->__("links.delete_board") ?></a>
                                    <?php } ?>
                                   </small>
                        <?php } ?>
                    </form>
            ...
            <div class="col-md-6">
                <div class="pull-right">
                    <div class="btn-group viewDropDown">
                        <button class="btn dropdown-toggle" data-toggle="dropdown"><?=$this->__("buttons.idea_wall_title") ?> <?=$this->__("links.view") ?></button>
        ...
        <?php if (count($this->get('allCanvas')) > 0) { ?>
            ...
            <div id="ideaMason" class="sortableTicketList">
                ...
                <?php foreach ($this->get('canvasItems') as $row) { ?>
                    ...
                    <div class="ticketBox idea-grid" id="item_<?php echo $row["id"]; ?>" data-value="<?php echo $row["id"]; ?>">

                        <div class="row">
                            <div class="col-md-12">

                            <div class="dropdown ticketDropdown userDropdown noBg show left lastDropdown dropRight">
                                ...
                                <?php  if ($login::userIsAtLeast("developer")) { ?>
                                    <div class="inlineDropDownContainer" style="float:right;">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            <li><a href="<?=BASE_URL ?>/ideas/delCanvasItem/<?php echo $row["id"]; ?>" style="color:#000;" class="delete ideaModal idea-modal-delete-btn" data="item_<?php echo $row["id"];?>"> <?php echo $this->__("links.delete_canvas_item"); ?></a></li>

                                        </ul>
                                    </div>
                                <?php } ?>

                                <h5><a href="<?=BASE_URL ?>/ideas/ideaDialog/<?php echo $row["id"]; ?>" class="ideaModal idea-title"
                                       data="item_<?php echo $row["id"]; ?>"><?php $this->e($row["description"]); ?></a></h5>
                ...
                <?php if (count($this->get('allCanvas')) > 0) { ?>
                    <a href="<?=BASE_URL ?>/ideas/ideaDialog?type=idea" class="ideaModal" style="color: lightgray;" id="customersegment">
                <div class="ticketBox idea-grid center" style="cursor:pointer!important">
                <div class="col-md-12">
                <i class="fas fa-4x fa-bullhorn" style="padding-top: 30px; color:lightgray"></i>
                <br><br>
                <span><i class="fas fa-plus"></i></span><?php echo $this->__("buttons.add_idea") ?>
                <?php } ?>
            </div>
                </div>
                </a>
                ...
                <div class="modal-content">
                    <form action="" method="post">
                        ...
                        <div class="modal-footer">
                            ...
                            <input type="submit" class="btn btn-primary" value="<?php echo $this->__("buttons.create_board")?>" name="newCanvas"/>
                        </div>
                    </form>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
